...
- **Departure 3: five crops changed.** Jack Smith's seal crop had been set by eye and caught half the seal and the letterhead rule; now measured and centred. PSI and Hillsborough re-cropped tighter so they read at 92px. Litvinenko and 9/11 trimmed of a page margin, a legend fragment, and a printed caption that showed at header size. Details and before-crops in `sources.yaml`.
- **Departure 4: the phone header wraps.** The navbar used to hide the wordmark under 30rem and show only the seal; with the seal gone the full name stays, and the nav beside it had been squeezed into a column five rows tall. It now wraps onto its own rows under the name.
- **Row image caps are px, not %.** A percentage `max-height` inside the grid slot does not resolve, and the first wired-up pass let Deepwater's portrait plate run 140px tall in a 92px row. Caught in the rendered page, not the tests.
